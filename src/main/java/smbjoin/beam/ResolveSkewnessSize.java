...
public abstract class ResolveSkewnessSize
    extends PTransform<
        PCollection<KV<Integer, KV<byte[], byte[]>>>, PCollectionView<List<Integer>>> {
            ...
            ParDo.of(
                    new DoFn<KV<Integer, KV<byte[], byte[]>>, KV<Integer, Long>>() {
                      ...
                      public void apply(
                        ...
                        int bucketId = Math.floorMod(value.getKey(), c.sideInput(numBucketsView()));
                        c.output(
                            KV.of(bucketId, value.getValue().getValue().length + recordOverhead()));
                      }
                    })
            ...
            ParDo.of(
                    new DoFn<KV<Void, Iterable<KV<Integer, Integer>>>, List<Integer>>() {
                      ...
                      public void processElement(
                          @Element KV<Void, Iterable<KV<Integer, Integer>>> value,
                          ProcessContext c) {
