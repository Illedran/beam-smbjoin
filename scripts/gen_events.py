...
def generate(c, k, fraction, schema_file, dir_path):
    ...
    with tempfile.NamedTemporaryFile(delete=False) as f:  # Atomic write
        ...
        os.replace(f.name, os.path.join(dir_path, f"events-{c}-{int(fraction * 100)}.avro"))


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('-c', '--count', metavar='C', type=int, help="Number of records to generate.", required=True)
    parser.add_argument('-k', '--key-size', metavar='K', type=int, help="Default length of key.", required=True)
    parser.add_argument('-f', '--skewed-fraction', metavar='F', type=float,
                        help="Default fraction of skewed keys. (default: %(default)s)", default=0.0)
    parser.add_argument('--schema-file', metavar='path', type=str, help="Path to schema file. (default: %(default)s)",
                        default="events_schema.json")
    parser.add_argument('--dir', metavar='dir_path', type=str,
                        help="Save file to a directory. (default: %(default)s)", default='.')
    args = parser.parse_args()

    generate(args.count, args.key_size, args.skewed_fraction, args.schema_file, args.dir)
