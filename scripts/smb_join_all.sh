...
ZIPF_SHAPES="0.00 0.10 0.20 0.30 0.40 0.50 0.60 0.70 0.80"
...
for i in ${ZIPF_SHAPES}; do
  INPUT_KEYS_BUCKETED="${DATA_BUCKET}/bucketed_keys/*.avro"
  INPUT_EVENTS_BUCKETED="${DATA_BUCKET}/bucketed_events/s$i/*.avro"

  sStr=${i/./}
  time=$(date +%s)

  target/pack/bin/smb-join-job --jobName="smb-join-job-s$sStr-$time-$( printf "%04x%04x" $RANDOM $RANDOM )" --events=${INPUT_EVENTS_BUCKETED} --keys=${INPUT_KEYS_BUCKETED} ${DATAFLOW_ARGS}
done;
